...
Papa.parse('https://cors-anywhere.herokuapp.com/'+csvCasos, config2);
...
Papa.parse('https://cors-anywhere.herokuapp.com/'+planilha, config);
...
function mostraMunicipios(results){
    ...
    //buscando o select para colocar os municipios
    ...
    for (var i = 0; i < results.data.length; i++) {
        ...
        //incluindo o municipio no select
        var option = document.createElement("option");
        option.text = a.MUNICIPIO;
        option.value = i+1;
        option.setAttribute("data-tokens",a.MUNICIPIO);
    ...
    //montando o combobox com os municipios
...
function selecionaMunicipio(select){
    var index = select.value;

    var sede = municipios.find((a)=>{return a.id==index});
    
    mymap.setView([sede.latitude,sede.longitude],8);
